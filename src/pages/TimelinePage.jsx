  ...
  return (
    <div>
      <div className="px-[clamp(18px,4vw,48px)] pb-1.5 pt-8">
        <h2 className="font-serif text-[1.5rem] text-indigo dark:text-sand">Heritage timeline</h2>
      ...
      <div className="mx-[clamp(18px,4vw,48px)] mb-[50px] border-l-2 border-black/10 pl-[26px] dark:border-white/10">
        {events.map((e, i) => (
          <div key={i} className="relative pb-7">
            ...
            <button onClick={() => navigate(`/state/${e.s.id}/${e.d.id}/${e.p.id}`)} className="font-bold text-indigo underline decoration-black/15 hover:decoration-teal dark:text-sand">
              {e.p.name}
            </button>
